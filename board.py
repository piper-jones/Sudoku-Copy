...
class Board:
        ...
        for row in range(9):
            for col in range(9):
                if self.locked_board[row][col] == 0 and self.current_board[row][col] != 0:
                    text = font.render(str(self.current_board[row][col]), True, purp, None)
                    self.screen.blit(text, (35 + (100 * col), 15+(100 * row)))
    ...
    def reset_to_original(self):
        self.current_board = self.locked_board
    ...
    def check_board(self):
        ...
        for i in range(9):
            if not self.is_valid_set(board[i]) or not self.is_valid_set([row[i] for row in board]):
                return False

        for i in range(0, 9, 3):
            for j in range(0, 9, 3):
                if not self.is_valid_set([board[row][col] for row in range(i, i + 3) for col in range(j, j + 3)]):
                    return False

        return True

    # Check if a list of numbers is a valid set (no duplicates and only numbers from 1 to 9)
